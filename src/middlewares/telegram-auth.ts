...
// SOURCE: https://github.com/Telegram-Mini-Apps/tma.js/blob/master/packages/init-data-node/src/validate.ts
...
export const verifyTelegramAuth = async (
  ...
  if (!initData || typeof initData !== 'string') {
    next(new HttpBadRequestError('Invalid init-data', ['No init-data header']));

    return;
  }

  try {
    validate(initData, environment.botToken);
  } catch (e) {
    next(new HttpBadRequestError('Invalid init-data', [e.message]));
